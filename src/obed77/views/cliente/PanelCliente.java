...
public class PanelCliente extends javax.swing.JPanel {
    static DefaultTableModel tableModelCli;
    /**
  ...
  {
    try {
      tableModelCli = (DefaultTableModel)tablaClientes.getModel();
      
      tableModelCli.setRowCount(0);
      Object[] fila = new Object[tableModelCli.getColumnCount()];
      ClienteSession provSession = new ClienteSession();
      ArrayList<ClienteTo> lista = provSession.getClientes();
      for (ClienteTo to : lista) {
        fila[0] = to.getTipoDocumento();
        fila[1] = to.getDocumento();
        fila[2] = to.getNombre();
        fila[3] = to.getTelefono();
        fila[4] = to.getCorreo();
        fila[5] = to.getDireccion();
        fila[6] = to.getEstatusString();
        tableModelCli.addRow(fila);
      }
      filtro("");
      ...
      tablaClientes.setModel(tableModelCli);
    }
  }
  ...
  private void validarSeleccionFila() {
    int fsel = tablaClientes.getSelectedRow();
    if (fsel != -1) {
      String estatus = tablaClientes.getValueAt(fsel, 6).toString();
      ClienteTo cat = new ClienteTo();
  ...
  {
    int fsel = tablaClientes.getSelectedRow();
    String tipDoc = tablaClientes.getValueAt(fsel, 0).toString();
    String doc = tablaClientes.getValueAt(fsel, 1).toString();
    String nombre = tablaClientes.getValueAt(fsel, 2).toString();
    try {
      int estatus = 0;
      ...
      session.modificarEstatusCliente(to);
      JOptionDialog.showMessageDialog("Cliente Inhabilitado Correctamente", "Clientes", 2);
      cargar();
    } catch (java.sql.SQLException ex) {
      LogService.logger.error(Principal.getUsuarioPrincipal().getUser(), "Error");
  ...
  private void habilitarCliente() {
    ...
    try {
      ...
      ClienteSession session = new ClienteSession();
      session.modificarEstatusCliente(to);
      JOptionDialog.showMessageDialog("Cliente Habilitado Correctamente", "Clientes", 2);
      cargar();
    } catch (java.sql.SQLException ex) {
      LogService.logger.error(Principal.getUsuarioPrincipal().getUser(), "Error");
      JOptionDialog.showMessageDialog(ErroresMap.MessageError(ex.getErrorCode(), nombre), "Error", 2);
  ...
  private static void filtro(String consulta) {
    TableRowSorter<DefaultTableModel> tr = new TableRowSorter(tableModelCli);
    ...
    private void initComponents() {
        ...
        tablaClientes.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Doc", "Número", "Nombre", "Teléfono", "Correo", "Dirección", "Estado"
            }
        ) {
        ...
        if (tablaClientes.getColumnModel().getColumnCount() > 0) {
            ...
            tablaClientes.getColumnModel().getColumn(2).setMinWidth(200);
            tablaClientes.getColumnModel().getColumn(3).setMinWidth(100);
            tablaClientes.getColumnModel().getColumn(3).setMaxWidth(100);
            tablaClientes.getColumnModel().getColumn(4).setMinWidth(200);
            tablaClientes.getColumnModel().getColumn(4).setMaxWidth(250);
            tablaClientes.getColumnModel().getColumn(5).setMinWidth(250);
            tablaClientes.getColumnModel().getColumn(6).setMinWidth(60);
            tablaClientes.getColumnModel().getColumn(6).setMaxWidth(75);
        }
    ...
    private void btn_opc_nuevoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_opc_nuevoActionPerformed
        ...
        NuevoCliente nuevo = new NuevoCliente((Frame) parentWindow, true);
...
// TODO add your handling code here:
    ...
    private void btn_opc_habilitarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_opc_habilitarActionPerformed
        btn_opc_habilitar.setOpaque(false);
        int opc = JOptionDialog.showConfirmDialog("¿Desea habilitar el cliente '" + tablaClientes.getValueAt(tablaClientes.getSelectedRow(), 1).toString() + "'?", "Habilitar Cliente", JOptionDialog.SI_NO_OPTION);
        if (opc == 0) {
            habilitarCliente();
        }
    ...
    private void btn_opc_inhabilitarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_opc_inhabilitarActionPerformed
        btn_opc_inhabilitar.setOpaque(false);
        int opc = JOptionDialog.showConfirmDialog("¿Desea inhabilitar el cliente '" + tablaClientes.getValueAt(tablaClientes.getSelectedRow(), 1).toString() + "'?", "Inhabilitar Cliente", JOptionDialog.SI_NO_OPTION);
        if (opc == 0) {
            inhabilitarCliente();
        }
        inhabilitarBotones();
    ...
    private void btn_opc_cerrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_opc_cerrarActionPerformed
        ...
        if (res == 0) {
            LogService.logger.info(Principal.getUsuarioPrincipal().getUser(), "Cerrando Panel Clientes");
    ...
    private void btn_opc_modificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_opc_modificarActionPerformed
        btn_opc_modificar.setOpaque(false);
        Window parentWindow = SwingUtilities.windowForComponent(this);
        ModificarCliente nuevo = new ModificarCliente((Frame) parentWindow, true);
        nuevo.setVisible(true);
        inhabilitarBotones();        // TODO add your handling code here:

...
public class PanelVentas extends javax.swing.JPanel {
    ...
    /**
     * Creates new form Panel_Clientees
     */
    public PanelVentas() {
        initComponents();
  ...
  {
    try {
      tableModelCli = (javax.swing.table.DefaultTableModel)tablaVentas.getModel();
      
      tableModelCli.setRowCount(0);
      Object[] fila = new Object[tableModelCli.getColumnCount()];
      VentaSession ventSession = new VentaSession();
      java.util.ArrayList<VentaTo> lista = ventSession.getVentas();
      for (VentaTo to : lista) {
        fila[0] = to.getDocumento();
        fila[1] = to.getReferencia();
        fila[2] = to.getFecha();
        fila[3] = to.getCliente();
        fila[4] = to.getUsuario();
        fila[5] = to.getTotalVenta();
      ...
      tablaVentas.setModel(tableModelCli);
    }
  }
  ...
  private void validarSeleccionFila() {
    int fsel = tablaVentas.getSelectedRow();
    if (fsel != -1) {
      String estatus = tablaVentas.getValueAt(fsel, 6).toString();
      VentaTo cat = new VentaTo();
  ...
  {
    int fsel = tablaVentas.getSelectedRow();
    String doc = tablaVentas.getValueAt(fsel, 0).toString();
    String ref = tablaVentas.getValueAt(fsel, 1).toString();
    try {
     
      VentaSession session = new VentaSession();
      session.anularVenta(doc,ref);
      JOptionDialog.showMessageDialog("Venta Anulada Correctamente", "Ventas", 2);
      cargar();
    } catch (java.sql.SQLException ex) {
      core.logger.LogService.logger.error(obed77.Principal.getUsuarioPrincipal().getUser(), "Error");
      obed77.views.dialogosComunes.JOptionDialog.showMessageDialog(core.controlador.principal.ErroresMap.MessageError(ex.getErrorCode(), doc+" "+ref), "Error", 2);
    } catch (Exception ex) {
      core.logger.LogService.logger.error(obed77.Principal.getUsuarioPrincipal().getUser(), "Error");
      obed77.views.dialogosComunes.JOptionDialog.showMessageDialog(core.controlador.principal.ErroresMap.MessageError(9999,""), "Error", 2);
    }
  ...
  private static void filtro(String consulta) {
    javax.swing.table.TableRowSorter<javax.swing.table.DefaultTableModel> tr = new javax.swing.table.TableRowSorter(tableModelCli);
    ...
    private void initComponents() {
        ...
        tablaVentas.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Documento", "Referencia", "Fecha", "Cliente", "Usuario", "Total Venta", "Estado"
            }
        ) {
        ...
        if (tablaVentas.getColumnModel().getColumnCount() > 0) {
            ...
            tablaVentas.getColumnModel().getColumn(0).setMaxWidth(90);
            tablaVentas.getColumnModel().getColumn(1).setPreferredWidth(100);
            tablaVentas.getColumnModel().getColumn(1).setMaxWidth(120);
            tablaVentas.getColumnModel().getColumn(2).setPreferredWidth(80);
            tablaVentas.getColumnModel().getColumn(2).setMaxWidth(90);
            ...
            tablaVentas.getColumnModel().getColumn(5).setMaxWidth(150);
            tablaVentas.getColumnModel().getColumn(6).setMinWidth(60);
            tablaVentas.getColumnModel().getColumn(6).setMaxWidth(75);
        }
    ...
    private void btn_opc_nuevoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_opc_nuevoActionPerformed
        ...
        NuevaVenta nuevo = new NuevaVenta((Frame) parentWindow, true);
    ...
    private void btn_opc_anularActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_opc_anularActionPerformed
        btn_opc_anular.setOpaque(false);
        int opc = JOptionDialog.showConfirmDialog("¿Desea anular esta venta?", "Inhabilitar Cliente", JOptionDialog.SI_NO_OPTION);
        if (opc == 0) {
            anularVenta();
        }
        inhabilitarBotones();
    ...
    private void btn_opc_cerrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_opc_cerrarActionPerformed
        ...
        if (res == 0) {
            LogService.logger.info(Principal.getUsuarioPrincipal().getUser(), "Cerrando Panel Clientees");
    ...
    private void btnDetallesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDetallesActionPerformed
        ...
        Window parentWindow = SwingUtilities.windowForComponent(this);
        DetallesVenta nuevo = new DetallesVenta((Frame) parentWindow, true);
        nuevo.setVisible(true);
        inhabilitarBotones();
